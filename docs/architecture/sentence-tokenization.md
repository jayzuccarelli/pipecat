...
Pipecat uses NLTK's `punkt_tab` models for sentence detection. Importing the
text utilities does not import NLTK. Pipeline warm-up loads the English model
in a background thread; TTS services also prepare their selected model while
services connect. Models are cached by language.

Deployments should install the data at build time with
`python -m nltk.downloader punkt_tab`, or set `NLTK_DATA` to an existing data
directory. When the data is missing, the loader attempts to download it on
first use. Model preparation can therefore require network access unless the
data is provisioned ahead of time.
